...
public abstract class Connexion {
            ...
            switch ((char) c.get("CONSTRAINT_TYPE")) {
                case 'C':
                    if (c.get("SEARCH_CONDITION").toString().contains("IS NOT NULL")) {
                        colonnes.get(a).isNullable(false);
                    }
                    break;
                case 'P':
    ...
    public String[] getTypesTab(){
        try {
            ArrayList<String> lesTypes = new ArrayList<>();
            DatabaseMetaData metadata = connect.getMetaData();
            resultSet = metadata.getTypeInfo();
            while (resultSet.next()) {
                String typeName = resultSet.getString("TYPE_NAME");
                lesTypes.add(typeName);
                
            }
                        String[] array = lesTypes.toArray(new String[lesTypes.size()]);
            return array;
        } catch (SQLException ex) {
            Logger.getLogger(Connexion.class.getName()).log(Level.SEVERE, null, ex);
            String[] array;
    ...
    public String[] getTablesTab(){
        try {
            ArrayList<String> listTables = new ArrayList<>(2);
            dbMetadata = connect.getMetaData();
            resultSet = dbMetadata.getTables(connect.getCatalog(), connect.getSchema(), "%", new String[]{"TABLE"});
            while (resultSet.next()) {
                String table = resultSet.getString("TABLE_NAME");
            ...
            String[] array = listTables.toArray(new String[listTables.size()]);
            return array;
        } catch (SQLException ex) {
            Logger.getLogger(Connexion.class.getName()).log(Level.SEVERE, null, ex);
            String[] array;
        ...
        try {
            ...
            dbMetadata = connect.getMetaData();
            resultSet = dbMetadata.getPrimaryKeys(connect.getCatalog(), connect.getSchema(), table);
            while (resultSet.next()) {
